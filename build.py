...
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' rx='7' fill='%230a0e14'/%3E%3Cpath d='M4 25 L12 11 L17 19 L21 13 L28 25 Z' fill='%23ffb24d'/%3E%3Cpath d='M10.4 14.5 L12 11 L13.6 14.5 L12 15.4 Z' fill='%23eaf1f5'/%3E%3C/svg%3E" />
<!-- P4.1: Splash (Inline, im <head> VOR dem großen Daten-Parse; file://-tauglich, kein Netz). -->
<style>
  #splash{position:fixed;inset:0;z-index:99999;background:#0a0e14;display:flex;flex-direction:column;
    align-items:center;justify-content:center;gap:16px;transition:opacity .5s ease}
  #splash.hide{opacity:0;pointer-events:none}
  #splash .sp-sig{width:96px;height:96px;animation:sp-pulse 1.6s ease-in-out infinite}
  #splash .sp-txt{color:#9fb0c0;font:600 15px/1 system-ui,-apple-system,sans-serif;letter-spacing:.5px}
  @keyframes sp-pulse{0%,100%{opacity:.55;transform:scale(.97)}50%{opacity:1;transform:scale(1)}}
  @media (prefers-reduced-motion: reduce){ #splash .sp-sig{animation:none} }
</style>
__HEAD_LIBS__
<style>
  :root{
  ...
  *{box-sizing:border-box}
  ...
  /* B16: auch eingeklappt EINE Breite (--panel-w) — nach P4.2 wuchs die Mini-Zeile
     inhaltsgetrieben ueber die Panel-Kante; tm-kpi ellipsiert den Ueberhang. */
  ...
  /* B17: klickbare Legende — dezente Hover-Affordance, kein eigener Button-Block */
  #legend .lfoc{cursor:pointer;border-radius:5px;margin:0 -4px;padding-left:4px;padding-right:4px}
  #legend .lfoc:hover{background:rgba(95,208,197,.12);color:var(--accent2)}
  ...
  /* B15: Body-Hoehe zusaetzlich an die Title-Card gekoppelt (--p-body-max, JS sizeCov):
     Panel dockt UNTER der Card an, nie ueberlagern; Rest scrollt intern. */
  #panel .body{padding:11px 14px 14px;font-size:15px;line-height:1.5;
    max-height:min(58vh, var(--p-body-max, 58vh));overflow-y:auto;   /* P1/Review §5: Grundschrift +1pt */
  ...
  #panel.as-cov{top:auto;bottom:calc(var(--row-h) + 54px);left:16px;width:var(--panel-w);
    max-height:min(calc(100vh - 130px), var(--p-panel-max, calc(100vh - 130px)));
...
<div id="title">
  ...
  <div id="legend" class="open">
    ...
    <div class="ll">
      <!-- B17: Legende klickbar = Sektor-Fokus (fitBounds in den freien Ausschnitt) -->
      <div class="lcols">
        <div class="lcol">
          <div class="lgrp lfoc" onclick="legendFocus('west',event)" title="Westalpen einpassen">Westalpen</div>
          <div class="lrow lfoc" onclick="legendFocus('Nordwestalpen',event)" title="Sektor einpassen"><span class="sw-nw"></span>Nordwestalpen</div>
          <div class="lrow lfoc" onclick="legendFocus('S&uuml;dwestalpen',event)" title="Sektor einpassen"><span class="sw-sw"></span>S&uuml;dwestalpen</div>
        </div>
        <div class="lcol">
          <div class="lgrp lfoc" onclick="legendFocus('ost',event)" title="Ostalpen einpassen">Ostalpen</div>
          <div class="lrow lfoc" onclick="legendFocus('Nordostalpen',event)" title="Sektor einpassen"><span class="sw-no"></span>Nordostalpen</div>
          <div class="lrow lfoc" onclick="legendFocus('Zentralostalpen',event)" title="Sektor einpassen"><span class="sw-zo"></span>Zentralostalpen</div>
          <div class="lrow lfoc" onclick="legendFocus('S&uuml;dostalpen',event)" title="Sektor einpassen"><span class="sw-so"></span>S&uuml;dostalpen</div>
        </div>
      </div>
...
const map = new maplibregl.Map({
  ...
  minZoom: 5.0,
  // B17: moderat geweitet — die alten Grenzen erzwangen an breiten Viewports eine
  // Mindest-Zoomstufe, die das Home-fitBounds in den freien Ausschnitt klemmte
  // (Bogen-Westteil rutschte unters Panel). Maske deckt den Zugewinn ab.
  maxBounds: [[1.8,41.8],[19.5,50.2]],
  style:{
    version:8,
    glyphs:'__GLYPHS__',
    sources:{
...
// P4.1: Splash ausblenden, sobald die Karte den ersten Frame hat (load -> idle). Fallbacks,
// falls unter file:// ohne Netz die Tiles fehlen und 'idle' verzögert -> Splash MUSS weg.
(function(){
  const s=document.getElementById('splash'); if(!s) return;
  let done=false;
  const hide=()=>{ if(done) return; done=true; s.classList.add('hide'); setTimeout(()=>{ try{ s.remove(); }catch(_){ } }, 600); };
  try{ map.on('load', ()=>{ try{ map.once('idle', hide); }catch(_){ } setTimeout(hide, 2500); }); }catch(_){ }
  setTimeout(hide, 8000);   // harter Fallback (Netz aus)
})();
...
map.on('load',()=>{
  ...
  // B17: Initial-Ansicht = dasselbe dynamische Framing wie der Home-Button
  // (ALPS-Konstruktorwert ist nur der Vor-Fit-Startpunkt).
  try{ _fitFree(_alpsBbox(), 0); }catch(_){ }
  /* PRIV:START */
...
function openSts(feat, camMode){   // camMode: undef=Gate(§6a), 'force'=immer fliegen (Suche), 'nofly'=nie (Chronik)
  ...
  if(typeof sizeCov==='function') sizeCov();   // B15: Body-Deckel mit frischer Chrome-Messung (Rückzeile sichtbar)
...
// B17: Home/Initial = dynamisches fitBounds der Alpen-bbox in den FREIEN Ausschnitt
// (Panel-/Ebenen-Padding einrechnen) statt Fix-Zoom — 27" mit offener Liste hatte
// den Westteil des Bogens unter dem Panel. Nach der Bewegung wird das persistente
// B6/B11-Padding restauriert (fitBounds-Padding soll nicht kleben bleiben).
let _alpsBB=null;
function _bboxOf(feats){
  let mx=180,my=90,Mx=-180,My=-90,found=false;
  feats.forEach(f=>(function w(a){ if(typeof a[0]==='number'){ found=true;
    if(a[0]<mx)mx=a[0]; if(a[0]>Mx)Mx=a[0]; if(a[1]<my)my=a[1]; if(a[1]>My)My=a[1]; }
    else a.forEach(w); })(f.geometry.coordinates));
  return found?[[mx,my],[Mx,My]]:null;
}
function _alpsBbox(){ if(!_alpsBB) _alpsBB=_bboxOf(SOIUSA_STS.features); return _alpsBB; }
function _fitFree(bb, dur){
  if(!bb) return;
  const pad={top:60, bottom:72,   // Suchknopf oben / Scale-Bar unten; knapp, damit die Breite bindet
    left:Math.max(28,(typeof _panelPadPx==='function'?_panelPadPx():0)+8),
    right:Math.max(28,(typeof _ebenenPadPx==='function'?_ebenenPadPx():0)+8)};
  map.fitBounds(bb,{padding:pad, pitch:0, bearing:0, duration:dur==null?1200:dur, essential:true});
  map.once('moveend', ()=>{ try{ _applyPanelPad(); }catch(_){ } });
}
function overview(){
  closePanel();
  _autoPitch=true;
...
// B17-Feature (freigegeben): Legende klickbar — West/Ost-Überschrift + Sektor-Zeile
// fokussieren den Teil-Bogen (fitBounds mit demselben freien Ausschnitt).
function legendFocus(which, ev){
  if(ev){ ev.stopPropagation(); }   // Legende sitzt in der Title-Card (collapse-Handler!)
  const parts={west:['Nordwestalpen','Südwestalpen'], ost:['Nordostalpen','Zentralostalpen','Südostalpen']};
  const sel=parts[which]||[which];
  const feats=SOIUSA_STS.features.filter(f=>sel.includes(f.properties.settore));
  if(!feats.length) return;
  _autoPitch=true;
  _fitFree(_bboxOf(feats));
}
...
function sizeCov(){
  ...
  if(phone){ avail=Math.round(window.innerHeight*0.72); }
    ...
    // B15: Steckbrief-Deckel — Panel dockt UNTER der Card an, nie überlagern; Rest
    // scrollt intern. as-cov (bottom-verankert, flex): GESAMT-Hoehe deckeln via
    // --p-panel-max; Standard-Modus (inline top = Card-Unterkante+10): Body-Deckel
    // via --p-body-max (Chrome Kopf+Tabs+Rueckzeile am DOM gemessen).
    try{
      document.documentElement.style.setProperty('--p-panel-max',
        Math.max(180, window.innerHeight-(rowH+54)-top)+'px');
      const pnl=document.getElementById('panel');
      let chrome=104;
      if(pnl){
        const ph=pnl.querySelector('.ph'), tb=pnl.querySelector('.tabs'), pb=pnl.querySelector('.pback');
        chrome=(ph?ph.offsetHeight:62)+(tb?tb.offsetHeight:34)
              +((pb&&pb.offsetParent!==null)?pb.offsetHeight:0)+4;
      }
      document.documentElement.style.setProperty('--p-body-max',
        Math.max(120, window.innerHeight-top-chrome-16)+'px');
    }catch(_){}
  }
