...
map.on('load',()=>{
  ...
  map.addImage('hut-club',  houseIcon('#e2574c'), {pixelRatio:2});   // Alpenvereinshütte
  map.addImage('hut-other', houseIcon('#9fb0c0'), {pixelRatio:2});   // sonstige Berghütte
  ...
  map.addLayer({id:'osm-peaks', type:'symbol', source:'osm-peaks', minzoom:8,
    layout:{'visibility':'none','icon-image':'peak','icon-anchor':'bottom','icon-allow-overlap':false,
      ...
      'text-field':['step',['zoom'], '', 10.5, ['concat',['get','name'],'\n',['to-string',['get','ele']],' m']],
...
function toggleHuts(){
  ...
  ['osm-huts-club','osm-huts-other'].forEach(l=>map.setLayoutProperty(l,'visibility',v));
